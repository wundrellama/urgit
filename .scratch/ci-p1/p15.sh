...
REPO=${ERPIT_REPO:-erpit-p15}; CLONE="$TMP/clone-$REPO"
RUN_NAME=${ERPIT_RUN_NAME:-p15}
ERPIT=/var/home/michael/workspace/urbit/erpit
if [ "${1:-}" != verify ]; then
row "P15: ERPit for real — eight jobs under the daemon, candidate %passed, landed"
if [ "${1:-}" = retry ]; then
in_clone || exit 1
test -z "$(git status --porcelain)" || { echo 'retry clone is dirty' >&2; exit 1; }
test -z "$(git diff a6d15ed HEAD -- . ':!README.md')" || { echo 'retry workload changed beyond README' >&2; exit 1; }
echo '-- Rider 4: unchanged workload, second README-only commit on the retained fixture clone'
else
...
echo "export CID=$CID; export OID=$OID; export T0=$T0" > "$TMP/$RUN_NAME-run.env"
fi   # end of the push half; `p15.sh verify` resumes from p15-run.env
if [ "${1:-}" = verify ]; then source "$TMP/$RUN_NAME-run.env"; row "P15 (verify): resumed on candidate $CID"; fi
...
echo "export ERPIT_CID=$CID; export ERPIT_OID=$OID; export ERPIT_T=$(( T1 - T0 ))" > "$TMP/$RUN_NAME.env"
end_row P15
