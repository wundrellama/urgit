...
::
::  +peer-mode-note: one line naming the chosen transfer mode and its size
::
++  peer-mode-note
  |=  [mode=peer-transfer-mode objects=@ud bytes=@ud pages=@ud]
  ^-  @t
  %-  crip
  ;:  weld
    "serving "  (scow %ud objects)  " objects, "  (scow %ud bytes)
    " bytes as %"  (trip `@tas`mode)  " in "  (scow %ud pages)
    ?:(=(1 pages) " page" " pages")
  ==
::
::  +peer-mode-tang: the same line, addressed, for the pier log
::
++  peer-mode-tang
  |=  [transfer=@uv target=ship repository=@t note=@t]
  ^-  tang
  :_  ~
  :-  %leaf
  ;:  weld
    "urgit: peer-prepare "  (scow %uv transfer)  " -> "  (scow %p target)
    " "  (trip repository)  ": "  (trip note)
  ==
::
++  peer-directed
...
++  peer-prepare
  ...
  ?:  directed
    ...
    =/  note=@t  (peer-mode-note %archive object-count object-bytes 1)
    =.  peer-serving  (~(put by peer-serving) transfer.req archive-flight)
    =.  peer-activities
      (peer-activity-start (peer-serve-activity-id transfer.req) %serve %incoming target repository.req note)
    =/  final-cards=(list card)
      ...
      ==
    %-  (slog (peer-mode-tang transfer.req target repository.req note))
    :_  this
  ...
  ?.  streamable
    ...
    =/  note=@t  (peer-mode-note %pack object-count object-bytes (lent pages))
  ...
  =/  note=@t  (peer-mode-note %objects object-count object-bytes pages)
  =.  peer-serving  (~(put by peer-serving) transfer.req flight)
  =.  peer-stream-jobs
