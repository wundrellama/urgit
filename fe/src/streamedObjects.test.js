...
test('transfer-id capability negotiation preserves the legacy request wire and pack fallback', () => {
  ...
  assert.match(peerPrepare, /streamable=\?/)
  assert.match(peerPrepare, /\(peer-object-capable transfer\.req\)/)
  assert.match(peerPrepare, /\?=\(\^ objects\)/)
...
test('transfer modes and stream jobs are transient and reset on init and load', () => {
  assert.match(backend, /\+\$  peer-serve[\s\S]*?mode=\?\(%pack %objects\)/)
  assert.match(
    backend,
  ...
  assert.match(
    backend,
    /\+\$  peer-receive[\s\S]*?mode=\?\(%pack %objects\)[\s\S]*?assemblies=\(map oid:git peer-object-assembly\)/,
  )
  assert.match(
    backend,
...
test('legacy positional transfer constructors explicitly retain pack mode', () => {
  ...
  assert.match(peerPrepare, /packed-flight=peer-serve[\s\S]*?%pack/)
...
test('batch helpers enforce one-MiB fragments and 512-fragment/four-MiB pages lazily', () => {
  assert.match(peerObjectBatchCount, /\|=  objects=\(list \[oid:git object:git\]\)/)
  assert.match(peerObjectBatchCount, /=\(count 512\)/)
  assert.match(peerObjectBatchCount, /1\.048\.576/)
  assert.match(peerObjectBatchCount, /4\.194\.304/)
  ...
  assert.match(peerObjectBatchCount, /page-room=@ud[\s\S]*?\(sub 4\.194\.304 bytes\)/)
  ...
  assert.match(peerObjectBatch, /=\(count 512\)/)
  assert.match(peerObjectBatch, /1\.048\.576/)
  assert.match(peerObjectBatch, /4\.194\.304/)
...
test('begin-objects mirrors begin validation, selects object mode, and requests only page one', () => {
  ...
  assert.match(peerBeginObjects, /\/g\/x\/1\/urgit\/\/1\/fine/)
  ...
  assert.doesNotMatch(peerBeginObjects, /gulf|\(add revision/)
})
...
test('fragment pages reject malformed bounds and duplicate oid/offset pairs before accounting', () => {
  ...
  assert.match(peerObjectFragments, /\(gth \(lent fragments\) 512\)/)
  assert.match(peerObjectFragments, /\(gth page-bytes 4\.194\.304\)/)
...
test('received counts only completed unique objects and final import remains atomic', () => {
  ...
  assert.match(peerObjectFragments, /\?\.  =\(received\.next expected\.next\)[\s\S]*?next-revision=@ud/)
  assert.match(peerObjectFragments, /\?\.  =\(revision pages\.next\)[\s\S]*?peer-finish transfer/)
...
test('hostile object announcements and partial assembly state have hard limits', () => {
  ...
  assert.match(peerPrepare, /\(lte \(lent objects\) peer-stream-max-objects\)/)
...
test('object-mode cancellation yawns only the one issued page while pack mode stays unchanged', () => {
  assert.match(peerTransferYawns, /mode=\?\(%pack %objects\)/)
  assert.match(peerTransferYawns, /=\(%objects mode\)/)
  assert.match(peerTransferYawns, /revision=@ud\s+\+\(~\(wyt in completed\)\)/)
  assert.match(peerTransferYawns, /\(gth revision pages\)/)
